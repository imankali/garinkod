...
import { Activity, BadgeDollarSign, ClipboardCheck, FileClock, Landmark, ShieldAlert, UsersRound, Warehouse } from "lucide-react";
...
type Tab = 'overview' | 'orders' | 'team' | 'audit';
...
export default function Management() {
  ...
  useEffect(() => { if (tab === 'team') loadTeam(); if (tab === 'audit') loadAudit(); }, [tab, data?.viewer.is_superuser]);
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-8 dark:bg-[#052e22]"><div className="mx-auto max-w-7xl"><section className="rounded-3xl bg-gradient-to-l from-slate-950 via-emerald-950 to-emerald-700 p-7 text-white shadow-xl"><div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-sm font-bold text-lime-300">مرکز فرماندهی گرین کود</p><h1 className="mt-2 text-3xl font-extrabold">مدیریت عملیات، مالی و اعتماد</h1><p className="mt-3 text-sm text-emerald-100">کاربر فعلی: {data.viewer.username} · نقش‌ها: {data.viewer.is_superuser ? 'مالک سیستم' : data.viewer.groups.join('، ') || 'کارمند'}</p></div><button onClick={() => refetch()} className="rounded-xl bg-white/15 px-4 py-2.5 text-sm font-bold hover:bg-white/25">به‌روزرسانی داده‌ها</button></div></section><div className="mt-6 grid gap-6 lg:grid-cols-[230px_1fr]"><aside className="h-fit rounded-3xl border border-slate-200 bg-white p-3 shadow-sm dark:border-emerald-900 dark:bg-emerald-950 lg:sticky lg:top-5"><nav className="flex gap-2 overflow-x-auto lg:flex-col">{nav.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => setTab(id)} className={`flex shrink-0 items-center gap-3 rounded-2xl px-4 py-3 text-sm font-bold transition ${tab === id ? 'bg-emerald-600 text-white' : 'text-slate-600 hover:bg-emerald-50 dark:text-emerald-100 dark:hover:bg-emerald-900/50'}`}><Icon size={18} />{label}</button>)}</nav></aside><section>{tab === 'overview' && <Overview data={data} />}{tab === 'orders' && <Orders data={data.recent_orders} onMarkPaid={markPaid} />}{tab === 'team' && <Team owner={data.viewer.is_superuser} staff={staff} roles={roles} loading={loadingTeam} onSave={saveStaff} />}{tab === 'audit' && <Audit audit={audit} loading={loadingAudit} />}</section></div></div></main>;
}

function Overview({ data }: { data: ManagementDashboard }) { return <div className="space-y-6"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric icon={BadgeDollarSign} label="فروش پرداخت‌شده" value={formatPrice(data.metrics.paid_revenue)} /><Metric icon={ClipboardCheck} label="سفارش در انتظار بررسی" value={data.metrics.pending_orders.toLocaleString('fa-IR')} /><Metric icon={ShieldAlert} label="شکایت باز" value={data.metrics.open_complaints.toLocaleString('fa-IR')} /><Metric icon={Warehouse} label="موجودی کم" value={data.metrics.low_stock_products.toLocaleString('fa-IR')} /></div><section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-emerald-900 dark:bg-emerald-950"><h2 className="text-xl font-extrabold text-slate-800 dark:text-white">هشدارهای عملیاتی</h2><div className="mt-5 grid gap-3 sm:grid-cols-2">{data.alerts.map((alert) => <div key={alert.type} className="flex items-center justify-between rounded-2xl bg-slate-50 p-4 dark:bg-emerald-900/40"><span className="text-sm text-slate-600 dark:text-emerald-100">{alert.label}</span><strong className={alert.count ? 'text-rose-600' : 'text-emerald-600'}>{alert.count.toLocaleString('fa-IR')}</strong></div>)}</div></section></div>; }
