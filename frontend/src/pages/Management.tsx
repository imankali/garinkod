...
import { Activity, BadgeDollarSign, ClipboardCheck, FileClock, Gauge, Landmark, PenLine, ShieldAlert, UserCog, UsersRound, Warehouse } from "lucide-react";
import toast from "react-hot-toast";

import { managementApi } from "../api/services";
import ContentStudio from "../components/management/ContentStudio";
...
type Tab = 'overview' | 'moderation' | 'content' | 'orders' | 'users' | 'health' | 'team' | 'audit';
...
export default function Management() {
  ...
  if (isLoading) return <div className="flex min-h-[60dvh] items-center justify-center"><div className="text-center text-slate-500"><div className="mx-auto h-10 w-10 animate-spin rounded-full border-4 border-emerald-500 border-t-transparent" /> <p className="mt-4">در حال بارگذاری مرکز مدیریت...</p></div></div>;
  if (isError || !data) return <div className="mx-auto max-w-3xl px-[var(--page-gutter)] py-12"><section className="rounded-3xl border border-rose-200 bg-rose-50 p-7 text-rose-900 dark:border-rose-900 dark:bg-rose-950/30 dark:text-rose-100"><ShieldAlert size={30} /><h1 className="mt-4 text-2xl font-extrabold">دسترسی مدیریتی ندارید</h1><p className="mt-2 leading-7">این بخش فقط برای کاربران staff فعال است. مالک سایت می‌تواند از Django Admin یا نقش‌های مدیریتی، دسترسی کارمندان را تعیین کند.</p></section></div>;
  ...
  const nav = [
    ...
    // Publishing and moderating are the same job on the same rows, so the
    // content studio sits next to the queue instead of behind another URL.
    { id: 'content' as Tab, label: 'تولید محتوا', icon: PenLine },
  ...
  return <div className="min-h-dvh bg-slate-50 px-4 py-8 dark:bg-[#052e22]"><div className="mx-auto max-w-7xl"><section className="rounded-3xl bg-gradient-to-l from-slate-950 via-emerald-950 to-emerald-700 p-7 text-white shadow-xl"><div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-sm font-bold text-lime-300">مرکز فرماندهی گرین کود</p><h1 className="mt-2 text-fluid-3xl font-extrabold">مدیریت عملیات، مالی و اعتماد</h1><p className="mt-3 text-sm text-emerald-100">کاربر فعلی: {data.viewer.username} · نقش‌ها: {data.viewer.is_superuser ? 'مالک سیستم' : data.viewer.groups.join('، ') || 'کارمند'}</p></div><button onClick={() => refetch()} className="rounded-xl bg-white/15 px-4 py-2.5 text-sm font-bold hover:bg-white/25">به‌روزرسانی داده‌ها</button></div></section>{/*
  min-w-0 on both grid children: without it, the auto column refuses to
  shrink below the tab row's min-content (seven non-shrinking buttons ≈
  ۱۱۱۸px) and the whole console blows out of a phone screen.
*/}
<div className="mt-6 grid min-w-0 gap-6 lg:grid-cols-[230px_1fr]"><aside className="h-fit min-w-0 rounded-3xl border border-slate-200 bg-white p-3 shadow-sm dark:border-emerald-900 dark:bg-emerald-950 lg:sticky lg:top-5"><nav className="flex gap-2 overflow-x-auto lg:flex-col">{nav.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => setTab(id)} className={`flex shrink-0 items-center gap-3 rounded-2xl px-4 py-3 text-sm font-bold transition ${tab === id ? 'bg-emerald-600 text-white' : 'text-slate-600 hover:bg-emerald-50 dark:text-emerald-100 dark:hover:bg-emerald-900/50'}`}><Icon size={18} />{label}</button>)}</nav></aside><section className="min-w-0">{tab === 'overview' && <Overview data={data} onOpenModeration={() => setTab('moderation')} />}{tab === 'moderation' && <ModerationQueue />}{tab === 'content' && <ContentStudio />}{tab === 'orders' && <Orders data={data.recent_orders} onMarkPaid={markPaid} />}{tab === 'users' && <UserLevels viewerLevel={viewerLevel} />}{tab === 'health' && <HealthPanel />}{tab === 'team' && <Team owner={data.viewer.is_superuser} staff={staff} roles={roles} loading={loadingTeam} onSave={saveStaff} />}{tab === 'audit' && <Audit audit={audit} loading={loadingAudit} />}</section></div></div></div>;
}
