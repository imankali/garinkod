...
در اجرای Django هشدارهای محیطیِ شناخته‌شده دیده شد: `OPERATIONS_TOKEN` در محیط تست خالی است و `staticfiles/` قبل از `collectstatic` وجود ندارد. در suite با SQLite نیز یک log از `database table is locked: shop_cartitem` در مسیر concurrent cart دیده شد؛ با وجود آن log، assertionهای suite سبز و نتیجهٔ ۵۹۹ تست `OK` بود. این را نباید ظرفیت production تلقی کرد و concurrency باید روی PostgreSQL staging نیز سنجیده شود. تولید schema exit code صفر داشت اما ۱۳ warning و ۲۰ error مستندسازی‌شده در serializer annotation، viewهای function-based و collision نام‌ها گزارش کرد؛ این مورد backlog جداگانهٔ قرارداد OpenAPI است، نه نتیجهٔ این refactor. تست‌های مرورگر و Lighthouse به browser/runtime مستقل نیاز دارند و صرفاً از روی compile نباید سبز فرض شوند.
