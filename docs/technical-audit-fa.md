...
1. **زرین‌پال و تسویه آنلاین هنوز متصل نشده‌اند.** اکنون flow واقعی ثبت سفارش، آدرس، کد پیگیری، رزرو موجودی و هماهنگی کارشناس وجود دارد؛ اما باید مدل `PaymentAttempt`، callback verify امن، idempotency، انقضای پرداخت و آزادسازی موجودی رزروشده قبل از دریافت وجه آنلاین پیاده‌سازی شود. تا آن زمان سایت نباید ادعای «پرداخت آنلاین» داشته باشد.
...
3. **مسیرهای باقی‌ماندهٔ UI کامل نیستند.** سفارش و پیگیری سفارش اکنون پیاده‌سازی شده‌اند، اما علاقه‌مندی‌ها، تنظیمات، مجله، درباره و تماس هنوز محتوای نهایی یا flow کامل ندارند. یا پیاده‌سازی شوند یا از ناوبری حذف بمانند.
